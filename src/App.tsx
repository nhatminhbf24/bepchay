...
function RecipeDetail() {
  const { id } = useParams(); const { state, dispatch } = useApp(); const recipe = state.recipes.find((item) => item.id === id); const [servings, setServings] = useState(recipe?.servings || 2);
  if (!recipe) return <Empty icon={<BookOpen />} title="Không tìm thấy món" text="Món này có thể đã được ẩn." />;
  const progress = state.progress[recipe.id] || { ingredients: [], steps: [] };
  function toggleIngredient(ingredientId: string) {
    const ingredients = progress.ingredients.includes(ingredientId) ? progress.ingredients.filter((item) => item !== ingredientId) : [...progress.ingredients, ingredientId];
    dispatch({ type: 'recipe-progress', recipeId: recipe!.id, value: { ...progress, ingredients } });
  }
  function toggleStep(stepIndex: number) {
    const steps = progress.steps.includes(stepIndex) ? progress.steps.filter((item) => item !== stepIndex) : [...progress.steps, stepIndex];
    dispatch({ type: 'recipe-progress', recipeId: recipe!.id, value: { ...progress, steps } });
  }
  const ingredientsDone = recipe.ingredients.filter((item) => progress.ingredients.includes(item.id)).length;
  const stepsDone = progress.steps.length;
  return <><RecipeImage recipe={recipe} large /><section className="recipe-detail"><div className="detail-heading"><div><p className="eyebrow">{recipe.category}</p><h1>{recipe.name}</h1><p>{recipe.summary}</p></div><Link className="ghost" to={`/recipes/${recipe.id}/edit`}>Chỉnh sửa</Link></div><div className="status-note"><span className={`status-dot ${recipe.verification}`} />{recipe.verification === 'draft' ? 'Bản nháp — cần đối chiếu trước khi nấu' : recipe.verification === 'source-checked' ? 'Đã đối chiếu phương pháp' : 'Đã nấu thử'}</div><div className="detail-stats"><span><Clock3 /> Chuẩn bị {recipe.prepMinutes} phút</span><span><ChefHat /> Nấu {recipe.cookMinutes} phút</span>{recipe.advanceMinutes ? <span><CalendarDays /> Cần chuẩn bị trước {recipe.advanceMinutes} phút</span> : null}</div><div className="serving-control"><b>Khẩu phần</b><div className="servings"><button onClick={() => setServings(Math.max(1, servings - 1))}><Minus /></button><span>{servings} người</span><button onClick={() => setServings(servings + 1)}><Plus /></button></div></div><section className="detail-section"><div className="detail-section-heading"><h2>Nguyên liệu</h2><span>{ingredientsDone}/{recipe.ingredients.length} đã có</span></div><ul className="ingredient-list">{recipe.ingredients.map((item) => { const done = progress.ingredients.includes(item.id); return <li key={`${item.id}-${item.unit}`}><button type="button" className={done ? 'check-row done' : 'check-row'} onClick={() => toggleIngredient(item.id)} aria-pressed={done}><span className="row-check"><Check /></span><span className="ingredient-copy">{item.name}{item.note ? <small>{item.note}</small> : null}</span><b>{scaleAmount(item.amount, recipe.servings, servings)} {item.unit}</b></button></li>; })}</ul></section><section className="detail-section"><div className="detail-section-heading"><h2>Cách làm</h2><span>{stepsDone}/{recipe.steps.length} bước</span></div><ol className="steps">{recipe.steps.map((step, index) => { const done = progress.steps.includes(index); return <li key={step}><button type="button" className={done ? 'step-row done' : 'step-row'} onClick={() => toggleStep(index)} aria-pressed={done}><span>{index + 1}</span><p>{step}</p></button></li>; })}</ol></section><section className="source-note"><b>Ghi chú nội dung</b><p>{recipe.sourceNote}</p>{recipe.sources?.length ? <ul>{recipe.sources.map((source) => <li key={source.url}><a href={source.url} target="_blank" rel="noreferrer">{source.title}</a></li>)}</ul> : null}</section></section></>;
}
